...
const addDependency = async function(name, condition, includedDependencies, cwd, polyfillsRequired){
	// Lazily check for git URLs and the like
	if(condition.indexOf("://") >= 0 || condition.startsWith("file:/")){
		condition = ">=0.0.0";
	}
	...
	for(let i = 0; i < modulePaths.length; i += 1){
		...
		(moduleData.browserRequirifier.polyfills ?? []).forEach(v => {
			polyfillsRequired.add(v);
		});
		const excludedFiles = {};
		...
		for(const dependency in moduleData.dependencies){
			...
			await addDependency(dependency, moduleData.dependencies[dependency], includedDependencies, modulePath, polyfillsRequired);
		}
	}
...
const generateFileList = async function(basePath = process.cwd()){
	...
	const globalExcludedFiles = {};
	(requirifierConfig.excludedFiles || []).forEach((file) => {
		globalExcludedFiles[path.resolve(basePath, file)] = true;
	});
	...
	for(const listName in requirifierConfig.moduleList){
		...
		moduleList.dependencies = moduleList.dependencies || [];
		const polyfillsRequired = new Set();
		(moduleList.polyfills ?? []).forEach(v => {
			polyfillsRequired.add(v);
		});
		
		const excludedFiles = {};
		Object.assign(excludedFiles, globalExcludedFiles);
		(moduleList.excludedFiles || []).forEach((file) => {
		...
		for(let i = 0; i < moduleList.dependencies.length; i += 1){
			...
			await addDependency(dependency, version, moduleListLists.dependencies, basePath, polyfillsRequired);
		}
